...
- **Service:** Resend API
- **Custom domain:** `noreply@turkish.directory` configured in Resend + DNS
- **Magic links:** Sent by Supabase, but custom sender configured via Resend SMTP relay

### Inquiry Email Flow
When a German buyer submits a contact form on a storefront:
1. Email sent to the **Turkish company** (the storefront owner)
2. Email sent to the **superadmin** (Turgay) — CC or separate notification
3. Inquiry content includes: buyer name, email, message, storefront URL
...
### Superadmin
- Single superadmin account for Turgay — manually created in Supabase
- Email + password login (not magic link)
- Role: `superadmin` in `admin_roles` table

### Additional Admins
- Superadmin can create additional admin accounts via admin dashboard
- Admins get email + password credentials
- Role-based permissions for future expansion

### Admin Dashboard (`/admin`)
- Separate auth flow from regular user magic links
- Admin login form with email + password
- Features: view/manage storefronts, create admins, view analytics

---

## 13. Storefront Slug Strategy

- **Auto-generated** from company name + industry for uniqueness
- Example: `bursa-mobilya` (Bursa furniture), `denizli-tekstil` (Denizli textiles)
- URL-safe slugification (Turkish characters → Latin: `ş` → `s`, `ç` → `c`, `ı` → `i`)
- If slug collision exists, append number: `bursa-mobilya-2`
- User cannot customize slug (keeps it simple, avoids reserved route conflicts)

---

## 14. Phase 2-4 Roadmap (Not in Scope for This Spec)
...
- **Legal review** — Impressum/Datenschutz templates need German legal review. Using standard templates for Phase 1, revisit before go-live.
- **Domain status** — Currently parked. Nameserver configuration needed before launch.
